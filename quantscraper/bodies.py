...
def workday_body(token: str, path: str) -> str | None:
    """The description for one Workday posting.

    `token` is `tenant|wdN|site[|host]` and `path` is the `externalPath` the
...
def run(
    ...
    def work(row: sqlite3.Row) -> tuple[sqlite3.Row, str | None]:
        return row, FETCHERS[row["ats"]](row["token"], row["job_id"])
